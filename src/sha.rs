...
pub extern "C" fn sha512_hash_password(password: *const c_char) -> *mut c_char {
    ...
    let hashed_password: CString =
        CString::new(sha512_simple(string_password, &params).unwrap()).unwrap();
    return hashed_password.into_raw();
}

#[no_mangle]
pub extern "C" fn sha512_verify_password(
    password: *const c_char,
    hashed_password: *const c_char,
) -> bool {
    let string_password = unsafe {
        assert!(!password.is_null());
